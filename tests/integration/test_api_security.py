...
class TestAuthenticationSecurity:
    ...
    def test_concurrent_authentication_attempts(self, client, app, monkeypatch):
        ...
        mock_request_cert = lambda csr_pem, user_id=None, certificate_type='client', client_ip=None: mock_signed_cert
        monkeypatch.setattr(
            'app.routes.api.v1.request_signed_certificate',
            mock_request_cert
        )
        
        # Mock the key generation to avoid cryptographic operations in threads
        mock_key = MagicMock()
        mock_key.private_bytes.return_value = b'mock-private-key'
        mock_csr = MagicMock()
        mock_csr.public_bytes.return_value = b'mock-csr'
        monkeypatch.setattr(
            'app.routes.api.v1.generate_key_and_csr',
            lambda **kwargs: (mock_key, mock_csr)
        )
        ...
        # Launch 5 concurrent requests (reduced to minimize SQLite contention)
        threads = []
        for _ in range(5):
        ...
        # Under concurrent load, some requests may fail due to database contention
        # The important thing is that we don't get crashes or inconsistent behavior
        # Valid responses: 200 (success), 401 (auth failure due to contention), 500 (database error)
        valid_codes = [200, 401, 500]
        assert all(code in valid_codes for code in results), f"Unexpected response codes: {results}"
        
        # At least some requests should succeed (not all fail)
        success_count = sum(1 for code in results if code == 200)
        assert success_count > 0, f"No requests succeeded under concurrent load: {results}"
