...
class TestUserServiceOnly:
    ...
    def test_user_route_redirects_with_query_string(self, test_app):
        """User route redirects preserve query parameters."""
        test_app.config['USER_URL_BASE'] = 'http://user.example.com'
        
        with test_app.test_client() as client:
            response = client.get('/user-only?param=value&other=test')
            assert response.status_code == 301
            assert 'param=value&other=test' in response.headers['Location']


class TestRedirectAdminToAdminService:
    """Test redirect_admin_to_admin_service decorator."""
    
    def test_non_admin_user_no_redirect(self, test_app):
        """Non-admin users access route normally."""
        with test_app.test_client() as client:
    ...
    def test_admin_user_redirects_when_admin_url_base_configured(self, test_app):
        """Admin users get redirected directly to admin service when ADMIN_URL_BASE configured."""
        test_app.config['ADMIN_URL_BASE'] = 'http://admin.example.com'
        ...
        with test_app.test_client() as client:
            ...
            # Should redirect directly to admin service
            assert response.headers['Location'] == 'http://admin.example.com/redirect-admin'
    
    def test_system_admin_user_redirects(self, test_app):
        """System admin users get redirected to admin service."""
        test_app.config['ADMIN_URL_BASE'] = 'http://admin.example.com'
    ...
    def test_auditor_user_redirects(self, test_app):
        """Auditor users get redirected to admin service."""
        test_app.config['ADMIN_URL_BASE'] = 'http://admin.example.com'
    ...
    def test_admin_no_redirect_when_admin_url_base_not_configured(self, test_app):
        """Admin users access route normally when service separation not configured."""
        # No ADMIN_URL_BASE configured
    ...
    def test_redirect_preserves_query_parameters(self, test_app):
        """Admin redirects preserve query parameters in target URL."""
        test_app.config['ADMIN_URL_BASE'] = 'http://admin.example.com'
        ...
        with test_app.test_client() as client:
            ...
            response = client.get('/redirect-admin?filter=active&page=2')
            assert response.status_code == 302
            
            # Check that target URL includes query params
            location = response.headers['Location']
            assert location == 'http://admin.example.com/redirect-admin?filter=active&page=2'
    
    def test_redirect_admin_no_admin_url_base_non_admin_user(self, test_app):
        """Test non-admin user accesses redirect route when no ADMIN_URL_BASE - no redirect."""
        # No ADMIN_URL_BASE configured
        
        with test_app.test_client() as client:
            with client.session_transaction() as sess:
                sess['user'] = {'sub': 'user123', 'is_admin': False, 'is_system_admin': False, 'is_auditor': False}
            
            response = client.get('/redirect-admin')
            assert response.status_code == 200
            assert response.data == b'shared content'
    
    def test_redirect_admin_non_admin_user_with_admin_url_base(self, test_app):
        """Test non-admin user accesses redirect route when ADMIN_URL_BASE configured - no redirect."""
        test_app.config['ADMIN_URL_BASE'] = 'http://admin.example.com'
